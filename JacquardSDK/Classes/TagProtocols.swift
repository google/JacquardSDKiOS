...
public protocol SubscribableTag {
  ...
  ///
  /// The data packet (without sequence number) will be published everytime a packet is received on
  /// the RawData characteristic of the tag.
  ...
  /// Subscribe to the data channel for raw data packets.
  ///
  /// The data packet (including sequence number and ack) will be published everytime a packet is
  /// received on the RawData characteristic of the tag.
...
public protocol ConnectedTag: JacquardTag {
  ...
  /// Each call to this function results in one signal strength value being published by the
  /// `rssiPublisher` publisher.
  func readRSSI()

  /// Send data to the Tag over the command characteristic.
  /// The expected format is the output of creating a protobuf message, with all the required
  /// fields for that particular message populated.
  ///
  /// - Parameter data: data to be sent to the Tag. This is a fragment of the entire message we
  ///                   send to the Device, to be assembled and processed after we're done writing.
  func send(data: Data)
}
