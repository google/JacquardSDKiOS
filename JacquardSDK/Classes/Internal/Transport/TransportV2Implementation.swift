...
enum CoreBluetoothError: Int {
  // Reference: https://developer.apple.com/documentation/corebluetooth/cbatterror/code/writenotpermitted
  case writeNotPermitted = 3

  // Reference: https://developer.apple.com/documentation/corebluetooth/cberror/code/peerremovedpairinginformation
  case peerRemovedPairingInfo = 14
}
...
class TransportV2Implementation: Transport {
  ...
  private let rawDataPacketSubject = PassthroughSubject<Data, Never>()
  private let rawBytesSubject = PassthroughSubject<[UInt8], Never>()

  var notificationPublisher: AnyPublisher<Data, Never> {
  ...
  var rawDataPacketPublisher: AnyPublisher<Data, Never> {
    return rawDataPacketSubject.eraseToAnyPublisher()
  }
  ...
  func send(data: Data) {
    requestResponseQueue.async {
      let fragments = self.transportState.commandFragmenter.fragments(fromPacket: data)
      for fragment in fragments {
        jqLogger.debug(
          "Send over 'DataTransfer characteristic': \(fragment.count):\(fragment.hexEncodedArray())"
        )
        self.peripheral.writeValue(
          fragment, for: self.characteristics.rawDataCharacteristic, type: .withoutResponse)
      }
    }
  }
  ...
  func deliverNotification(packet: Data) {
    notificationCacheLock.lock()
    if cacheNotifications {
      jqLogger.debug("caching notification packet")
    ...
    notificationSubject.send(packet)
  }
...
extension TransportV2Implementation: PeripheralDelegate {
  ...
  func peripheral(
    ...
    guard characteristic.uuid == JacquardServices.commandUUID else {
      ...
      invalidateCommandExecutionTimer()
      didWriteCommandSubject.send(TagConnectionError.internalError)
      return
    }
        ...
        && error.code == CoreBluetoothError.writeNotPermitted.rawValue
    {
      // Write characteristic not supported.
      invalidateCommandExecutionTimer()
      // Remove current request as this error will be handled and command will be resent using
      // write without response characteristic type.
      pendingRequests.removeFirst()
    }
  ...
  func peripheral(
    ...
    case JacquardServices.rawDataUUID:
      ...
      if let packet = self.transportState.rawDataFragmenter.packet(fromAddedFragment: data) {
        processRawDataPacket(packet: packet)
        rawDataPacketSubject.send(packet)
      }
