...
public struct ListModulesCommand: CommandRequest {
  ...
  /// :nodoc:
  public func parseResponse(outerProto: Any) -> Result<[Module], Error> {
...
public struct ActivateModuleNotificationSubscription: NotificationSubscription {
  ...
  /// :nodoc:
  public func extract(from outerProto: Any) -> ModuleStatus? {
    ...
    return innerProto.status == .ok ? .activated : .deactivated
  }
}
