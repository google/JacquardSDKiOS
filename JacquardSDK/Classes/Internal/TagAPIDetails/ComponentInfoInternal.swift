...
extension ComponentInfoCommand {
  ...
  public var request: V2ProtocolCommandRequestIDInjectable {
    ...
    request.domain = componentID == TagConstants.FixedComponent.tag.rawValue ? .base : .gear
    request.opcode =
      componentID == TagConstants.FixedComponent.tag.rawValue ? .deviceinfo : .gearInfo
  ...
  public func parseResponse(outerProto: Any) -> Result<ComponentInfo, Error> {
    ...
    return .success((ComponentInfo(version: firmwareVersion, uuid: info.uuid)))
  }
}
