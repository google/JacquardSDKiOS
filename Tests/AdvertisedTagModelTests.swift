...
class AdvertisedTagModelTests: XCTestCase {
  ...
  func testInitAdvertisedTagModel() {
    ...
    let advertisedTag =
      AdvertisedTagModel(peripheral: peripheral, advertisementData: advertisementData, rssi: -72.0)
    ...
    XCTAssertEqual(advertisedTag?.displayName, deviceName)
    XCTAssertEqual(advertisedTag?.rssi, -72.0)
  }

  func testInitTagWithoutAdvertisingData() {
    let peripheral = FakePeripheralImplementation(identifier: uuid, name: deviceName)
    let emptyAdvertisementData = [String: Any]()
    let advertisedTag = AdvertisedTagModel(
      peripheral: peripheral,
      advertisementData: emptyAdvertisementData,
      rssi: 0.0
    )
  ...
  func testDecodeSerialNumber() {
    ...
    for (serial, data) in serialNumbers {
      ...
      let advertisedTag =
        AdvertisedTagModel(peripheral: peripheral, advertisementData: advertisingData, rssi: -72.0)
