...
final class TransportV2Tests: XCTestCase {
  ...
  func testVerifyNotifyCharacteristic() {
    ...
    transport.notificationPublisher
      .buffer(size: Int.max, prefetch: .byRequest, whenFull: .dropOldest)
      .sink { packet in
        let notification = try? self.parseNotification(packet)
  ...
  func testVerifyMalformedDataDeliveryNotification() {
    ...
    transport.notificationPublisher
      .buffer(size: Int.max, prefetch: .byRequest, whenFull: .dropOldest)
      .sink { packet in
        guard let _ = try? self.parseNotification(packet) else {
          malformedDataExpectation.fulfill()
          return
        }
...
extension TransportV2Tests {
  func parseNotification(_ packet: Data) throws -> Google_Jacquard_Protocol_Notification {
