...
let config = SDKConfig(apiKey: "AIzaSyChhaFF-23vLgTS_1-jQD-y-5GzxYfgBWY")
...
final class FirmwareUpdateStateMachineTests: XCTestCase {
  ...
  func testApplyAndExecuteUpdateStateMachineSuccess() throws {
    ...
    enqueueBatteryStatusRequest(transport)
  ...
  func testFirmwareApplyUpdateWhenLowBattery() throws {
    ...
    updateStateMachine.applyUpdates()
    enqueueBatteryStatusRequest(transport, lowBattery: true)
...
extension FirmwareUpdateStateMachineTests {
  ...
  private func enqueueBatteryStatusRequest(_ transport: FakeTransport, lowBattery: Bool = false) {
