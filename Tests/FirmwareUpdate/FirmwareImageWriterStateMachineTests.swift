...
final class FirmwareImageWriterStateMachineTests: XCTestCase {
  ...
  func testDFUStopWriting() {
    ...
    let preparingForWriteExpectation = expectation(description: "preparingForWriteExpectation")
    let writingExpectation = expectation(description: "writingExpectation")
    let writingStoppedExpectation = expectation(description: "writingStoppedExpectation")
    // Image size / Chunk size: 1000 / 128 ~ 8
    writingExpectation.expectedFulfillmentCount = 8
    ...
    imageWriterStateMachine.statePublisher.sink { state in
      ...
      case .writing(let bytesWritten):
        // User might interrupt writing at the middle of process,
        // calling stop writing to interrupt writing process
        // by using check for any random #bytes written.
        if bytesWritten > 800 {
          do {
            try imageWriterStateMachine.stopWriting()
          } catch let error as NSError {
            XCTFail("Stop Wriring failed: \(error)")
          }
        }
        writingExpectation.fulfill()
      case .stopped:
        writingStoppedExpectation.fulfill()
      default:
        XCTFail("Failed with state \(state)")
      }
    }.addTo(&observations)
    ...
    wait(
      for: [writingExpectation, writingStoppedExpectation],
      timeout: 1.0,
      enforceOrder: true
    )
  }

  // TODO(b/240654101): Write test cases to veify Stop Updates on multiple tags.
}
