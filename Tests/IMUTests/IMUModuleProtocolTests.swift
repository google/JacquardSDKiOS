...
class IMUModuleProtocolTests: XCTestCase {
  ...
  func testIMUProtocolStaticSamplingRates() {
    let lowSampling = IMUSamplingRate.low
    XCTAssertEqual(lowSampling.accelerometer.rate, AccelerometerSamplingRate.hertz25.rate)
    XCTAssertEqual(lowSampling.gyroscope.rate, GyroscopeSamplingRate.hertz25.rate)

    let midSampling = IMUSamplingRate.mid
    XCTAssertEqual(midSampling.accelerometer.rate, AccelerometerSamplingRate.hertz100.rate)
    XCTAssertEqual(midSampling.gyroscope.rate, GyroscopeSamplingRate.hertz200.rate)

    let highSampling = IMUSamplingRate.high
    XCTAssertEqual(highSampling.accelerometer.rate, AccelerometerSamplingRate.hertz400.rate)
    XCTAssertEqual(highSampling.gyroscope.rate, GyroscopeSamplingRate.hertz800.rate)
