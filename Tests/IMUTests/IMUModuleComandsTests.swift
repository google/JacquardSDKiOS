...
class IMUModuleComandsTests: XCTestCase {
  ...
  func testSetIMUConfigCommand() {

    let testSamplingRate = IMUSamplingRate(accelerometer: .low, gyroscope: .low)
    let config = Google_Jacquard_Protocol_ImuConfiguration.with {
      $0.accelSampleRate = testSamplingRate.accelerometer.rate
      $0.gyroSampleRate = testSamplingRate.gyroscope.rate
    }
    let command = IMUConfigCommand(imuConfig: config)
    XCTAssertNotNil(command)
    ...
    let descriptor = request.Google_Jacquard_Protocol_UJTConfigWriteRequest_configWrite.imuConfig
    XCTAssertEqual(descriptor.accelSampleRate, testSamplingRate.accelerometer.rate)
    XCTAssertEqual(descriptor.gyroSampleRate, testSamplingRate.gyroscope.rate)
  }
}
