...
class IMUParserTests: XCTestCase {
  ...
  func testIMUParserForFirstSample() throws {

    let testBundle = Bundle.test
    guard
      let path = testBundle.path(forResource: "imu1", ofType: "bin"),
  ...
  func testIMUParserForAllSamplesAndEndMarkerParsed() throws {
    let testBundle = Bundle.test
    guard
      let path = testBundle.path(forResource: "imu2", ofType: "bin"),
      let stream = InputStream(fileAtPath: path)
    else {
      throw IMUParsingError.fileCouldNotBeOpened
    }
    ...
    // Check if End Marker was found when parsing.
    XCTAssertEqual(completed, true)

    // Check the count for number of samples.
    XCTAssertEqual(samples.count, 2619)
  }
}
