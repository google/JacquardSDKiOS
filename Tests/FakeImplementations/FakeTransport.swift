...
class FakeTransport: Transport {
  var rawDataPacketPublisher = PassthroughSubject<Data, Never>().eraseToAnyPublisher()
  ...
  private var notificationSubject = PassthroughSubject<Data, Never>()
  ...
  func postGearAttach() {
    let notification = Google_Jacquard_Protocol_Notification.with { outerProto in
      ...
      outerProto.opcode = .attached
      outerProto.domain = .base
      outerProto.componentID = 0
    }

    let data = try! notification.serializedData()
  ...
  func postDFUExecute() {
    let notification = Google_Jacquard_Protocol_Notification.with { notification in
      ...
      notification.domain = .base
      notification.opcode = .deviceinfo
      notification.componentID = 0
    }
    let data = try! notification.serializedData()
    notificationSubject.send(data)
  }
