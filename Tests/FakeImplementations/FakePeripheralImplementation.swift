...
class FakePeripheralImplementation: Peripheral {
  ...
  /// Battery status command response.
  private let batteryStatusResponse = Data([192, 13, 8, 5, 16, 0, 24, 0, 202, 7, 4, 8, 49, 16, 2])

  /// DFU status command response.
  private let dfuStatusResponse = Data(
    [
      192, 26, 8, 6, 16, 0, 24, 0, 194, 12, 17, 8, 0, 16, 168, 202, 11, 24, 223, 215, 2, 32, 128,
      193, 11, 40, 237, 36,
    ]
  )

  /// DFU prepare command response..
  private let dfuPrepareCommandResponse = Data([192, 6, 8, 7, 16, 0, 24, 0])

  /// DFU write command response for last segment.
  private let dfuWriteCommandResponse = Data(
    [192, 17, 8, 215, 1, 16, 0, 24, 0, 210, 12, 7, 8, 176, 202, 11, 16, 206, 5]
  )

  /// DFU execute command response.
  private let dfuExecuteCommandResponse = Data([192, 6, 8, 50, 16, 0, 24, 0])
  ...
  /// ComponentInfo command response data representing version 1.0.6.
  let componentInfoResponseBadFirmwarePart1 = Data(
    [
      128, 79, 8, 3, 16, 0, 24, 0, 186, 6, 70, 10, 11, 71, 111, 111, 103, 108, 101, 32, 73, 110, 99,
      46, 18, 3, 85, 74, 84, 24, 4, 34, 24, 48, 45, 48, 55, 45, 57, 51, 50, 52, 70, 76, 72, 66, 75,
      48, 48, 48, 51, 82, 45, 49, 57, 48, 51, 40, 1, 48, 0,
    ]
  )

  // ComponentInfo command response remaining data representing version 1.0.6.
  let componentInfoResponseBadFirmwarePart2 = Data(
    [
      65, 56, 6, 64, 1, 72, 0, 80, 0, 88, 200, 225, 224, 139, 1, 96, 160, 207, 239, 193, 2,
    ]
  )
  ...
  /// Post component info response for bad firmware 1.68.0
  func postUpdateForComponentInfoResponseForBadFirmware() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {

      // Create response characteristic with success data.
      let responseCharacteristicPart1 = FakeCharacteristic(
        responseValue: self.componentInfoResponseBadFirmwarePart1)
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristicPart1, error: nil)

      let responseCharacteristicPart2 = FakeCharacteristic(
        responseValue: self.componentInfoResponseBadFirmwarePart2)
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristicPart2, error: nil)
      self.completionHandler?(self, .didUpdateValue(.componentInfoCommand))
    }
  }

  func postUpdateForBatteryStatusCommandResponse() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      // Create response characteristic with success data.
      let responseCharacteristic = FakeCharacteristic(responseValue: self.batteryStatusResponse)

      // When receives success response for given command.
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristic, error: nil)
      self.completionHandler?(self, .didUpdateValue(.batteryCommand))
    }
  }

  func postUpdateForDFUStatusCommandResponse() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      // Create response characteristic with success data.
      let responseCharacteristic = FakeCharacteristic(responseValue: self.dfuStatusResponse)

      // When receives success response for given command.
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristic, error: nil)
      self.completionHandler?(self, .didUpdateValue(.dfuStatus))
    }
  }

  func postUpdateForDFUPrepareCommandResponse() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      // Create response characteristic with success data.
      let responseCharacteristic = FakeCharacteristic(responseValue: self.dfuPrepareCommandResponse)

      // When receives success response for given command.
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristic, error: nil)
      self.completionHandler?(self, .didUpdateValue(.dfuPrepare))
    }
  }

  func postUpdateForDFUWriteCommandResponse() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      // Create response characteristic with success data.
      let responseCharacteristic = FakeCharacteristic(responseValue: self.dfuWriteCommandResponse)

      // When receives success response for given command.
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristic, error: nil)
      self.completionHandler?(self, .didUpdateValue(.dfuWrite))
    }
  }

  func postUpdateForDFUExecuteCommandResponse() {
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      // Create response characteristic with success data.
      let responseCharacteristic = FakeCharacteristic(responseValue: self.dfuExecuteCommandResponse)

      // When receives success response for given command.
      self.delegate?.peripheral(self, didUpdateValueFor: responseCharacteristic, error: nil)
      self.completionHandler?(self, .didUpdateValue(.dfuExecute))
    }
  }
